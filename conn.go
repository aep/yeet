...
func fromBuilder(b *Builder, u *url.URL, ssock net.Conn) (*Conn, error) {
...
func (c *Conn) connect1() (net.Conn, error) {
	...
	host := c.url.Host
	if c.url.Port() != "" {
	} else if c.url.Scheme == "ws" {
		host += ":80"
	} else if c.url.Scheme == "wss" {
		host += ":443"
	} else if c.url.Scheme == "http" {
		host += ":80"
	} else if c.url.Scheme == "https" {
		host += ":443"
	} else {
		host += ":80"
	}
...
func (c *Conn) ensureSock(deadline time.Time) (net.Conn, error) {
	...
	for {
		...
		c2, err := c.connect1()
		if err == nil {
			c.log.Print("yeet: connected to: ", c.url)
			c.sock = c2
			return c2, nil
		}
